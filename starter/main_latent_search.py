...
def trial():
    ...
    with open(
        "output/results/" + f"{Config.search_type}_{Config.search_number_iterations}" + ".csv", "a", newline=""
    ) as csvfile:
...
if __name__ == '__main__':
    ...
    search_iterations = [30, 40, 50] #[10, 20, 30, 40, 50]

    for num_iterations in search_iterations:
        Config.search_number_iterations = num_iterations
        with open(
            "output/results/" + f"{Config.search_type}_{Config.search_number_iterations}" + ".csv", "w+", newline=""
        ) as csvfile:
            logger = csv.writer(csvfile, delimiter=",", quoting=csv.QUOTE_MINIMAL)
